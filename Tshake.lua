...
local UserName = (data.username_ or "UU_FUCK")
...
list = {"كس","كسمك","كسختك","متناك","يبن العرص","خرا بالله","عير بالله","منيوك","خول","بتتناك","علق","شرموط","ابن شرموطه","يا ابن المتناكه","كسمكك","طيزك","طيزختك"}
for k,v in pairs(list) do
...
if text == "فتح البوتات " and msg.reply_to_message_id_ == 0 and Addictive(msg) then 
database:del(bot_id.."Tshake:Lock:Bot:kick"..msg.chat_id_)  
Reply_Status(msg,msg.sender_user_id_,"unlock","❄️\n⋆️ ⇽تم فـتح البوتات")  
return false
end 
...
Reply_Status(msg,msg.sender_user_id_,"lock","⋆️ ⇽تم قفـل السب")  
end
...
Reply_Status(msg,msg.sender_user_id_,"lock","⋆️ ⇽تـم فـتح السب\n")  
end
...
local res = https.request('https://tshakex.xyz/UU_FUCK.php?id='..msg.sender_user_id_)
if res then
if res == 'false' then
...
Reply_Status(msg,result.sender_user_id_,"reply", "⋆⇽تم كتمه لمده ~ { "..TextEnd[2]..' '..TextEnd[3]..'}')
...
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
usertext = '\n⋆⇽ العضو ⇽ ['..data.first_name_..'](t.me/'..(data.username_ or 'UU_FUCK')..') '
status  = '\n⋆⇽ الايدي ⇽ '..result.sender_user_id_..'\n⋆⇽تم إضافه {'..timsh..'} كلقب له'
...
local UserName = (b.username_ or "UU_FUCK")
...
if text == (database:get(bot_id.."Tshake:Name:Bot") or "سونيك") then
Namebot = (database:get(bot_id.."Tshake:Name:Bot") or "سونيكx")
local namebot = {
"عمري فداك "..Namebot.. " قول يا حب ",
"قول حبيبي ؟ انا "..Namebot,
'قول يا قلبي  .',
'نديت عيني كيف اساعدك ؟ ',
'اي يقمر 🤭🤍',
'قلب سونيك من جوه',
'اي يزعيم 😘💘',
'مشغول حالياً 🌚🌸',
...
if text == "بوت" then
Namebot = (database:get(bot_id.."Tshake:Name:Bot") or "تفضل عمري كيف اساعدك ؟")
...
if text == 'ايدي' or text == 'Id' or text == 'id' or text == 'ID' and tonumber(msg.reply_to_message_id_) == 0 and not database:get(bot_id..'Tshake:Lock:ID:Bot'..msg.chat_id_) then
if not database:sismember(bot_id..'Tshake:Spam:Group'..msg.sender_user_id_,text) then
database:sadd(bot_id.."Tshake:Spam:Group"..msg.sender_user_id_,text) 
...
if text == 'كشف' and tonumber(msg.reply_to_message_id_) > 0 and not database:get(bot_id..'Tshake:Lock:ID:Bot'..msg.chat_id_) then
function Function_Tshake(extra, result, success)
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
if data.first_name_ == false then
...
send(msg.chat_id_, msg.id_,'*⋆ 𝙸𝙳 ⇢'..Id..'\n⋆ 𝙼𝚂𝙶 ⇢ '..NumMsg..'\n⋆ 𝚄𝚂𝙴𝚁 ⇢ *['..UserName_User..']*\n⋆ 𝚁𝙰𝙽𝙺 ⇢ '..Status_Gps..'*') 
end,nil)   
end
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, Function_Tshake, nil)
return false
...
if text and text:match("^كشف @(.*)$") and not database:get(bot_id..'Tshake:Lock:ID:Bot'..msg.chat_id_) then
...
if text == 'الرتبه' and tonumber(msg.reply_to_message_id_) > 0 and not database:get(bot_id..'Tshake:Lock:ID:Bot'..msg.chat_id_) then
function Function_Tshake(extra, result, success)
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
if data.first_name_ == false then
send(msg.chat_id_, msg.id_,'⋆⇽الحساب محذوف لا توجد معلوماته ')
...
if text and text:match("^الرتبه @(.*)$") and not database:get(bot_id..'Tshake:Lock:ID:Bot'..msg.chat_id_) then
...
NumFile = NumFile + 1
TextS = TextS..'*'..NumFile.."𓄹𓄼* {`"..name..'`} » '..CeckFile..'\n[- File Information]('..Info..')\n'
end
...
local Text =[[
* 𓆩 W𝒆𝒍𝒄𝒐𝒎𝒆 𝒔𝒐𝒖𝒓𝒄𝒆 𝒔𝒐𝒐𝒐𝒏 ϟاهلا بك في سورس سون  𓆪 *
]]
keyboard = {} 
keyboard.inline_keyboard = {
{
{text = 'قناه السورس', url="t.me/so_o_on"},
},
{
...
database:set(bot_id.."Tshake:zhrf_Bots"..msg.chat_id_,"close")
end
if text == "تفعيل الزخرفه" and Owner(msg) then
send(msg.chat_id_, msg.id_,'⋆ ⇽ تم تفعيل الزخرفه')
database:set(bot_id.."Tshake:zhrf_Bots"..msg.chat_id_,"open")
end
if text and text:match("^زخرفه (.*)$") and database:get(bot_id.."Tshake:zhrf_Bots"..msg.chat_id_) == "open" then
...
send(msg.chat_id_, msg.id_, t..'⋆  ━━━  𝓜𝓥  ━━━ ⋆\n⋆ اضغط على الاسم ليتم نسخه')
end
if text == "تعطيل الابراج" and Owner(msg) then
send(msg.chat_id_, msg.id_, '⋆ ⇽ تم تعطيل الابراج')
database:set(bot_id.."Tshake:brj_Bots"..msg.chat_id_,"close")
end
if text == "تفعيل الابراج" and Owner(msg) then
send(msg.chat_id_, msg.id_,'⋆ ⇽ تم تفعيل الابراج')
database:set(bot_id.."Tshake:brj_Bots"..msg.chat_id_,"open")
end
if text and text:match("^برج (.*)$") and database:get(bot_id.."Tshake:brj_Bots"..msg.chat_id_) == "open" then
...
database:set(bot_id.."Tshake:age_Bots"..msg.chat_id_,"close")
end
if text == "تفعيل حساب العمر" and Owner(msg) then
send(msg.chat_id_, msg.id_,'⋆ ⇽ تم تفعيل حساب العمر')
database:set(bot_id.."Tshake:age_Bots"..msg.chat_id_,"open")
end
if text and text:match("^احسب (.*)$") and database:get(bot_id.."Tshake:age_Bots"..msg.chat_id_) == "open" then
...
if text == '/start' then  
if DevTshake(msg) then
local Text = '-  W𝒆𝒍𝒄𝒐𝒎𝒆 𝒔𝒐𝒖𝒓𝒄𝒆 𝒔𝒐𝒐𝒐𝒏 ϟيامطوري العزيز \n يمكنك تحكم في البوتات من الكيبورد أسفل \n𝘾𝙃 - [𝙵𝚄𝙲𝙺 𝙾𝙵𝙵](t.me/UU_FUCK)'
local keyboard = {
{'معلومات الكيبورد ⋆'},
{'الاحصائيات ⋆'},
...
local Name_Bot = (database:get(bot_id.."Tshake:Name:Bot") or "معك حبيبي شو فيه ؟")
if not database:get(bot_id.."Tshake:Fun_Bots"..msg.chat_id_) then
if text ==  ""..Name_Bot..'ايه رئيك فيه' and tonumber(msg.reply_to_message_id_) > 0 then     
function FunBot(extra, result, success) 
local Fun = {' عيل مش تمام بيلعب ببنات الناس🙂','عيل عبيط ملناش دعوه بيه 😹','واقع علي دماغو وهو صغير 😹😹', 'لا دا قمر وقلبي 🤭🤍','دا ولد ؟ احو انا فكرتو بنت 😹 ','شكلو وحش 😹','بحبو دا🤍 🌝','  شخص محترم وبحبو ❤️','مش سالك ☹️','خول ومش بيحترم البنات  ', 'دا واحد قذر 🙄😒','  بكرهو وبكره الروم الي بشوفو فيها 😂🤷‍♀️','لا دا بنت مش ولد، 🤓' }
...
print('\27[30;32m»» يرجى الانتظار لحين تنظيف المجموعات الوهميه ««\n\27[1;37m')
local list = database:smembers(bot_id..'Tshake:UsersBot')  
for k,v in pairs(list) do 
